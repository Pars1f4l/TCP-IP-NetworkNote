...
Windows示例中通过send函数和recv函数完成了数据交换，因此只需要补充设置可选项相关示例。  
由于Windows中不存在Linux那样的信号处理机制，在Windows中需要使用select函数来处理这个问题。select函数能够监视哪些套接字发生了异常。异常是不同寻常的数据流，收到Out-of-band数据也属于异常，示例如下。  

[oob_send_win.c](oob_send_win.c)  
上面示例只是oob_send.c的windows移植版。

[oob_recv_win.c](oob_recv_win.c)  
Windows中没有与writev和readv直接对应的函数，但可以通过重叠I/O得到相同的结果。
...
3. **通过 recv 函数验证输入缓冲中是否存在数据时（确认后立即返回时），如何设置 recv 函数最后一个参数中的可选项？分别说明各可选项的含义**。
   ...
   答：使用 MSG_PEEK 和MSG_DONTWAIT来验证输入缓冲中是否存在待接收的数据。各个可选项的意义参见上面对应章节的表格。
